...
const editOptions=['더 연하게','더 진하게','더 심플','파츠 줄이기','포인트 추가','자석감 강하게','오로라감 추가','성수 무드','강남 무드','청순하게','화려하게','실물시술 쉽게','가격 낮춰서','이달의 아트용','오마카세용'];
const inventory=[
...
function renderOpsToday(){
 ...
 const stats=[
   ['오늘 예약',salonAppointments.length+'건','대기 '+(salonAppointments.length-done)],
   ['예상 매출',won(total)+'원','완료 '+won(completedSales)+'원'],
   ['빈 시간','2개','15:30 · 19:30'],
   ['재방문','6명','신규 2명']
 ];
 const box=$('#todayStats');if(box)box.replaceChildren(...stats.map(([k,v,s])=>{const a=document.createElement('article');a.className='today-stat panel';a.innerHTML=`<span>${k}</span><b>${v}</b><small>${s}</small>`;return a}));
 if($('#todayProgress'))$('#todayProgress').textContent=`${done}/${salonAppointments.length} 완료 · 다음 예약을 바로 확인하세요`;
 const timeline=$('#opsTimeline');if(timeline){timeline.innerHTML='';salonAppointments.forEach(a=>{
   const row=document.createElement('button');row.className='ops-appointment '+(a.status==='완료'?'done':'');
   row.innerHTML=`<time>${a.time}</time><div><b>${a.customer}</b><small>${a.service} · ${a.staff}</small></div><strong>${won(a.amount)}원</strong><span>${a.status}</span>`;
   row.onclick=()=>openOpsDetail(a);timeline.appendChild(row);
 })}
 const next=salonAppointments.find(x=>x.status!=='완료')||salonAppointments[0];
 const n=$('#nextCard');if(n&&next)n.innerHTML=`<span class="mini-label">NEXT</span><div class="next-time">${next.time}</div><h3>${next.customer}</h3><p>${next.service} · ${next.staff}</p><div class="next-tags"><span>${next.note}</span><span>${next.membership}</span></div><button class="primary-btn" id="nextOpenBtn">예약 열기</button>`;
...
function renderBooking(){
 ...
 const tabs=$('#staffTabs');if(tabs){tabs.innerHTML='';staffNames.forEach(s=>{const b=document.createElement('button');b.textContent=s;b.classList.toggle('active',bookingStaff===s);b.onclick=()=>{bookingStaff=s;renderBooking()};tabs.appendChild(b)})}
 if($('#bookingDateLabel'))$('#bookingDateLabel').textContent=bookingDayOffset===0?'오늘 · 9월 22일':bookingDayOffset>0?`+${bookingDayOffset}일`:`${bookingDayOffset}일`;
 const hours=$('#bookingHours');if(hours){hours.innerHTML='';for(let h=10;h<=21;h++){const d=document.createElement('div');d.textContent=String(h).padStart(2,'0')+':00';hours.appendChild(d)}}
 const list=$('#bookingList');if(list){list.innerHTML='';const data=salonAppointments.filter(a=>bookingStaff==='전체'||a.staff===bookingStaff);data.forEach(a=>{const b=document.createElement('button');b.className='booking-block '+(a.status==='완료'?'done':'');const start=10*60;const [hh,mm]=a.time.split(':').map(Number);const mins=hh*60+mm-start;b.style.top=`${Math.max(0,mins/60*64)+8}px`;b.innerHTML=`<b>${a.time} ${a.customer}</b><small>${a.service}</small><span>${a.staff} · ${won(a.amount)}원</span>`;b.onclick=()=>openOpsDetail(a);list.appendChild(b)})}
}
...
function setView(v){
 state.view=v;$$('.screen').forEach(x=>x.classList.remove('active'));const target=$(`#${v}Screen`);if(!target)return;target.classList.add('active');
 ...
 const titles={opsHome:'오늘, 필요한 것만.',booking:'예약',customers:'고객',more:'더보기',home:'ART STUDIO',create:'6안 만들기',library:'ART 보관함',collection:'이달의 아트',settings:'DNA · 재료'};
 $('#pageTitle').textContent=titles[v]||'LUDIA NAIL';
 if(v==='create')state.start=Date.now();
...
function startFromHome(){const v=$('#homePrompt').value.trim();state.draft.homePrompt=v;setView('create');if(v){$('#conceptInput').value=v;state.draft.concept=v}updateBrief();schedulePersist()}
...
function recentCard(d){const a=document.createElement('article');a.className='recent-card';a.innerHTML=`<img src="${d.img}" alt="${d.name}"><div class="card-copy"><b>${d.name}</b><small>${d.tags.slice(0,2).join(' · ')}</small></div>`;a.onclick=()=>openSheet(d);return a}
...
function generatedSet(){const timeLimit=+$('#maxTime').value;const target=+$('#targetPrice').value;return baseDesigns.map((d,i)=>({...d,id:Date.now()+i,time:Math.min(d.time,timeLimit),price:Math.min(d.price,target+10000),fit:Math.max(86,d.fit-(state.conditions.has('실물시술 쉽게')?0:2)),status:'후보',tags:[state.dna.split(' ')[0],...d.tags.slice(0,2)]}))}
function designCard(d,i){const a=document.createElement('article');a.className='design-card';const stockWarn=d.stock.includes('대체')||d.stock.includes('88');a.innerHTML=`<div class="design-media"><img src="${d.img}" alt="${d.name}"><span class="design-num">0${i+1}</span><button class="design-fav" data-fav>♡</button></div><div class="design-body"><div class="design-name-row"><b>${d.name}</b><span>${money(d.price)}</span></div><p>${d.desc}</p><div class="meta-pills"><span>${d.diff}</span><span>${d.time}분</span><span>실행 ${d.fit}%</span><span class="${stockWarn?'stock-warn':'stock-ok'}">${d.stock}</span></div><div class="design-footer"><button data-save>저장</button><button data-edit>수정</button></div></div>`;a.querySelector('[data-edit]').onclick=()=>openSheet(d);a.querySelector('.design-media').onclick=(e)=>{if(e.target.closest('[data-fav]'))return;openSheet(d)};a.querySelector('[data-save]').onclick=()=>saveDesign(d);a.querySelector('[data-fav]').onclick=(e)=>{e.currentTarget.textContent=e.currentTarget.textContent==='♡'?'♥':'♡';toast('즐겨찾기 상태를 바꿨어요')};return a}
function generate(){state.designs=generatedSet();schedulePersist();$('#resultSection').classList.remove('hidden');$('#designGrid').replaceChildren(...state.designs.map(designCard));toast('6개 시안을 준비했어요');setTimeout(()=>$('#resultSection').scrollIntoView({behavior:'smooth',block:'start'}),100);renderPicker()}
$('#generateMain').onclick=generate;$('#regenerateBtn').onclick=generate;function openCompare(){
 const grid=$('#compareGrid');grid.innerHTML='';
 state.designs.slice(0,6).forEach((d,i)=>{const warn=d.stock.includes('대체')||d.stock.includes('88');const c=document.createElement('button');c.className='compare-item';c.innerHTML=`<div class="compare-thumb"><img src="${d.img}" alt="${d.name}"><span>0${i+1}</span></div><div class="compare-copy"><b>${d.name}</b><div><span>${d.time}분</span><span>${money(d.price)}</span><span>${d.diff}</span></div><small class="${warn?'warn':''}">${d.stock} · 실행 ${d.fit}%</small></div>`;c.onclick=()=>{closeCompare();openSheet(d)};grid.appendChild(c)});
 $('#compareSheet').classList.add('open');$('#compareSheet').setAttribute('aria-hidden','false');document.body.style.overflow='hidden';
}
function closeCompare(){$('#compareSheet').classList.remove('open');$('#compareSheet').setAttribute('aria-hidden','true');document.body.style.overflow=''}
$('#compareBtn').onclick=openCompare;$$('[data-close-compare]').forEach(x=>x.onclick=closeCompare);
...
function ensureFingerLooks(d){
 ...
 FINGERS.forEach((f,i)=>{if(!d.fingerLooks[f])d.fingerLooks[f]={mods:[],crop:(i%5)*8}});
...
function renderHandEditor(){
 ...
 hands.forEach(([label,list])=>{const hand=document.createElement('div');hand.className='editor-hand';hand.innerHTML=`<span class="hand-label">${label}</span><div class="editor-fingers"></div>`;const row=hand.querySelector('.editor-fingers');
   list.forEach((f,i)=>{const look=state.active.fingerLooks[f];const b=document.createElement('button');b.type='button';b.className=`finger-photo ${fingerLookClasses(look.mods)} ${state.fingers.has('전체')||state.fingers.has(f)?'selected':''}`;b.dataset.finger=f;b.innerHTML=`<span class="finger-img" style="background-image:url('${state.active.img}');background-position:${20+(i*15)%70}% center"></span><small>${f.replace(/^L|^R/,'')}</small><i>${look.mods.length||''}</i>`;b.onclick=()=>toggleFingerFromPhoto(f);row.appendChild(b)});box.appendChild(hand)});
...
function applyLiveMod(mod){
 if(!state.active)return;ensureFingerLooks(state.active);const targets=selectedFingerNames();const allOn=targets.every(f=>state.active.fingerLooks[f].mods.includes(mod));
 targets.forEach(f=>{const arr=state.active.fingerLooks[f].mods;state.active.fingerLooks[f].mods=allOn?arr.filter(x=>x!==mod):[...new Set([...arr,mod])]});
 recalcDesign(state.active);renderHandEditor();syncEditChipStates();renderEditorMetrics();render3DStage();schedulePersist();toast(`${targets.length===10?'전체':targets.join(' · ')} · ${mod} ${allOn?'해제':'즉시 반영'}`);
}
...
function render3DStage(){
 ...
 FINGERS.forEach((f,i)=>{const look=state.active.fingerLooks[f];const n=document.createElement('div');n.className=`viewer-nail ${fingerLookClasses(look.mods)} v${i+1}`;n.innerHTML=`<span style="background-image:url('${state.active.img}');background-position:${20+(i%5)*15}% center"></span><small>${f}</small>`;hand.appendChild(n)});stage.appendChild(hand)
}
...
function renderLibrary(){const q=($('#librarySearch')?.value||'').trim().toLowerCase();const f=state.filter;const data=state.library.filter(d=>(f==='전체'||d.status===f||d.tags.includes(f))&&(!q||d.name.toLowerCase().includes(q)||d.tags.join(' ').toLowerCase().includes(q)));$('#libraryGrid').innerHTML='';data.forEach(d=>{const a=document.createElement('article');a.className='library-card';a.innerHTML=`<span class="status-badge">${d.status}</span><img src="${d.img}" alt="${d.name}"><div class="card-copy"><b>${d.name}</b><small>${d.diff} · ${d.time}분 · ${money(d.price)}</small><div class="tagline">${d.tags.slice(0,3).map(t=>`<span>#${t}</span>`).join('')}</div></div>`;a.onclick=()=>openSheet(d);$('#libraryGrid').appendChild(a)});$('#libraryEmpty').classList.toggle('hidden',data.length>0)}
...
function renderPicker(){const data=state.library.length?state.library:baseDesigns;$('#pickerGrid').innerHTML='';data.forEach(d=>{const a=document.createElement('article');a.className='picker-card'+(state.collection.has(d.id)?' selected':'');a.innerHTML=`<span class="picker-check">${state.collection.has(d.id)?'✓':'+'}</span><img src="${d.img}" alt="${d.name}"><b>${d.name}</b>`;a.onclick=()=>{if(state.collection.has(d.id))state.collection.delete(d.id);else if(state.collection.size<6)state.collection.add(d.id);else return toast('6개까지 선택할 수 있어요');renderPicker();renderCollectionPreview();schedulePersist()};$('#pickerGrid').appendChild(a)});const count=state.collection.size;$('#collectionCount').textContent=`${count} / 6`;$('#collectionProgress').style.width=`${count/6*100}%`}
function renderCollectionPreview(){const data=state.library.length?state.library:baseDesigns;const chosen=data.filter(d=>state.collection.has(d.id)).slice(0,6);const box=$('#previewGrid');box.innerHTML='';for(let i=0;i<6;i++){const d=chosen[i];const x=document.createElement('div');x.className='preview-item'+(d?'':' empty');x.innerHTML=d?`<img src="${d.img}" alt="${d.name}"><span class="preview-price">${money(d.price)}</span>`:`<span>${i+1}</span>`;box.appendChild(x)}}
...
$('#opsStatusBtn').onclick=()=>{if(!activeAppointment||activeAppointment.status==='완료')return;if(activeAppointment.status==='대기'){activeAppointment.status='진행중';toast('시술을 시작했어요')}else{activeAppointment.status='완료';toast('시술 완료로 변경했어요')}closeOpsDetail();renderOpsToday();renderBooking()};
...
$('#quickAddBooking').onclick=()=>toast('빠른 예약: 고객·시간·시술 3단계 입력 UI를 다음 연결 단계에서 서버와 묶어요');
$('#newCustomerBtn').onclick=()=>toast('고객 등록은 이름·연락처만 먼저 받고 나머지는 시술 후 채우는 방식으로 연결할게요');
$('#prevDay').onclick=()=>{bookingDayOffset--;renderBooking()};$('#nextDay').onclick=()=>{bookingDayOffset++;renderBooking()};
...
(async function boot(){const mode=await loadPersisted();hydrateFromState();renderAll();renderOpsToday();renderBooking();renderCustomers();setView('opsHome');setSaveStatus('saved',mode==='new'?'자동저장 준비':'자동저장됨');if(mode==='restored'||mode==='migrated')toast(mode==='migrated'?'기존 보관함을 새 저장방식으로 옮겼어요':'이전 작업을 복원했어요');if(mode==='new')schedulePersist()})();
