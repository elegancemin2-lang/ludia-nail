...
function renderBooking(){
 ...
 data.forEach(a=>{const col=Math.max(0,visibleStaff.indexOf(a.staff));if(col<0)return;const b=document.createElement('button');b.type='button';b.className='booking-block '+(a.status==='완료'?'done ':a.status==='진행중'?'progress ':'waiting ');const [hh,mm]=a.time.split(':').map(Number);const mins=hh*60+mm-startHour*60;const dur=a.duration||90;b.style.top=`${Math.max(0,mins/60*pxHour)+4}px`;b.style.height=`${Math.max(50,dur/60*pxHour-8)}px`;b.style.left=`calc(${col*100/visibleStaff.length}% + 6px)`;b.style.width=`calc(${100/visibleStaff.length}% - 12px)`;b.innerHTML=`<span class="booking-time">${a.time}</span><b>${a.customer}</b><small>${a.service}</small><em>${a.status==='완료'?'완료':won(a.amount)+'원'}</em>`;b.onclick=e=>{e.stopPropagation();openOpsDetail(a)};list.appendChild(b)});
