...
function renderBooking(){
 ...
 data.forEach(a=>{const col=Math.max(0,visibleStaff.indexOf(a.staff));if(col<0)return;const b=document.createElement('button');b.type='button';b.className='booking-block '+(a.status==='완료'?'done ':a.status==='진행중'?'progress ':a.status==='취소'||a.status==='노쇼'?'cancelled ':'waiting ');const [hh,mm]=a.time.split(':').map(Number);const mins=hh*60+mm-startHour*60;const dur=a.duration||90;b.style.top=`${Math.max(0,mins/60*pxHour)+4}px`;b.style.height=`${Math.max(50,dur/60*pxHour-8)}px`;b.style.left=`calc(${col*100/visibleStaff.length}% + 6px)`;b.style.width=`calc(${100/visibleStaff.length}% - 12px)`;b.innerHTML=`<span class="booking-time">${a.time}</span><b>${a.customer}</b><small>${a.service}</small><em>${a.status==='완료'?'완료':a.status==='취소'?'취소':a.status==='노쇼'?'노쇼':won(a.amount)+'원'}</em>`;b.onclick=e=>{e.stopPropagation();openOpsDetail(a)};list.appendChild(b)});
...
$('#quickAddBooking').onclick=()=>openQuickBooking('13:00',bookingStaff==='전체'?'루디아':bookingStaff);$('#homeQuickAdd')?.addEventListener('click',()=>{setView('booking');setTimeout(()=>openQuickBooking('13:00','루디아'),120)});
...
/* v2.36 · Nail Canvas Pro — functional single-finger paint board */
(()=>{
 const canvas=document.getElementById('nailDrawCanvas'),tip=document.getElementById('nailBigTip'),partsLayer=document.getElementById('nailPartsLayer');if(!canvas||!tip||!partsLayer)return;
 const ctx=canvas.getContext('2d');let currentFinger=null,drawing=false,last=null,brush='pen',brushSize=9,ink='#d998a6';
 const colors=[
 ['#fffaf8','밀키 화이트','all'],['#f7e9e6','베이비 핑크','nude'],['#f3d7d9','밀키 핑크','nude'],['#e9a9b4','로즈 시럽','syrup'],['#d98f9c','뮤트 로즈','syrup'],['#c96f82','베리 시럽','syrup'],['#ead7cb','피치 누드','nude'],['#dcc4b8','베이지 누드','nude'],['#cbaaa1','모카 누드','nude'],['#b7786f','로지 브라운','all'],['#9b283c','와인','all'],['#721a2b','딥 체리','all'],['#3b2428','에스프레소','all'],['#f3eee8','오프화이트','all'],['#d9d2c9','그레이지','all'],['#b9a8b9','라일락','all'],['#9eafd1','페리윙클','all'],['#768aa2','블루그레이','all'],['#8fae9a','세이지','all'],['#d8c989','버터','all'],['#c9b7dc','오로라','glitter'],['#e5c3d4','핑크 펄','glitter'],['#d9c7a4','샴페인','glitter'],['#b9b9bf','실버 글리터','glitter'],['#b7bec9','실버 자석','magnet'],['#d4a6b9','핑크 자석','magnet'],['#8d6b75','모브 자석','magnet'],['#7b8190','스틸 자석','magnet'],['#ae8c72','브론즈 자석','magnet'],['#584c65','플럼 자석','magnet']
 ];
 let fav=JSON.parse(localStorage.getItem('ludiaNailFavColors')||'["#f3d7d9","#e9a9b4","#ead7cb","#d4a6b9"]');
 let customParts=JSON.parse(localStorage.getItem('ludiaNailParts')||'[]');
 const baseParts=[['✦','크리스탈','stone'],['◆','다이아','stone'],['◇','오팔','stone'],['●','화이트 진주','pearl'],['◉','핑크 진주','pearl'],['♡','하트 메탈','metal'],['☆','스타 메탈','metal'],['☾','문 메탈','metal'],['🎀','리본','ribbon'],['୨୧','미니 리본','ribbon'],['✿','플라워','flower'],['❀','데이지','flower']];
 const fingerStore=f=>{if(!state.active||!f)return null;ensureFingerLooks(state.active);const look=state.active.fingerLooks[f];look.canvas=look.canvas||{drawing:null,parts:[]};look.canvas.parts=look.canvas.parts||[];return look};
 function saveDrawing(){const look=fingerStore(currentFinger);if(look)look.canvas.drawing=canvas.toDataURL('image/png')}
 function loadFinger(f){currentFinger=f;const look=fingerStore(f);if(!look)return;tip.style.setProperty('--pro-nail-base',look.base||'#f3d7d9');ctx.clearRect(0,0,canvas.width,canvas.height);if(look.canvas.drawing){const im=new Image();im.onload=()=>ctx.drawImage(im,0,0,canvas.width,canvas.height);im.src=look.canvas.drawing}renderPlacedParts();document.querySelector('.nail-canvas-empty span').textContent=f+' 상세 편집';document.getElementById('nailCanvasPro').classList.add('ready');document.getElementById('nailCanvasPro').scrollIntoView({behavior:'smooth',block:'start'})}
 window.LudiaNailCanvas={openFinger:loadFinger};
 function applyColor(hex){ink=hex;if(!currentFinger)return;const look=fingerStore(currentFinger);pushEditorHistory();look.base=hex;look.accent=hex;tip.style.setProperty('--pro-nail-base',hex);renderHandEditor();renderArtLiveStage();renderDirectStudioPreview();schedulePersist()}
 function renderColors(group='favorite'){const box=document.getElementById('nailProColors');box.innerHTML='';colors.filter(x=>group==='favorite'?fav.includes(x[0]):group==='all'||x[2]===group).forEach(x=>{const b=document.createElement('button');b.className='nail-pro-swatch';b.title=x[1]+' · 길게 눌러 즐겨찾기';b.innerHTML='<i style="background:'+x[0]+'"></i><small>'+x[1]+'</small>';let timer,long=false;b.onpointerdown=()=>{long=false;timer=setTimeout(()=>{long=true;fav.includes(x[0])?fav=fav.filter(v=>v!==x[0]):fav.push(x[0]);localStorage.setItem('ludiaNailFavColors',JSON.stringify(fav));renderColors(group);toast('즐겨찾기를 변경했어요')},520)};b.onpointerup=()=>{clearTimeout(timer);if(!long)applyColor(x[0])};b.onpointerleave=()=>clearTimeout(timer);box.appendChild(b)})}
 function partNode(p,index){const el=document.createElement('button');el.className='placed-nail-part';el.style.left=p.x+'%';el.style.top=p.y+'%';el.style.transform='translate(-50%,-50%) rotate('+p.r+'deg) scale('+p.s+')';el.innerHTML=p.data?'<img src="'+p.data+'" alt="">':'<span>'+p.icon+'</span>';let sx,sy,ox,oy,moved=false;el.onpointerdown=e=>{e.preventDefault();sx=e.clientX;sy=e.clientY;ox=p.x;oy=p.y;moved=false;el.setPointerCapture(e.pointerId)};el.onpointermove=e=>{if(!el.hasPointerCapture(e.pointerId))return;const r=partsLayer.getBoundingClientRect();p.x=Math.max(4,Math.min(96,ox+(e.clientX-sx)/r.width*100));p.y=Math.max(4,Math.min(96,oy+(e.clientY-sy)/r.height*100));moved=true;el.style.left=p.x+'%';el.style.top=p.y+'%'};el.onpointerup=()=>{if(!moved){p.r=(p.r+45)%360;p.s=p.s>=1.6?.7:p.s+.15}renderPlacedParts();schedulePersist()};el.ondblclick=()=>{fingerStore(currentFinger).canvas.parts.splice(index,1);renderPlacedParts();schedulePersist()};return el}
 function renderPlacedParts(){partsLayer.innerHTML='';const look=fingerStore(currentFinger);if(!look)return;look.canvas.parts.forEach((p,i)=>partsLayer.appendChild(partNode(p,i)))}
 function addPart(x){if(!currentFinger){toast('먼저 10손가락 중 하나를 선택해 주세요');return}const look=fingerStore(currentFinger);look.canvas.parts.push({icon:x.icon||'',data:x.data||'',name:x.name,x:50,y:48,r:0,s:1});renderPlacedParts();schedulePersist();toast(x.name+' 추가 · 드래그 이동 · 탭 회전/크기 · 더블탭 삭제')}
 ...
 canvas.onpointerdown=e=>{if(!currentFinger){toast('먼저 손톱 하나를 선택해 주세요');return}drawing=true;last=pos(e);canvas.setPointerCapture(e.pointerId);if(brush==='dot'){ctx.fillStyle=ink;ctx.beginPath();ctx.arc(last.x,last.y,brushSize*.9,0,Math.PI*2);ctx.fill()}};
 ...
 canvas.onpointerup=canvas.onpointercancel=()=>{if(drawing){drawing=false;last=null;saveDrawing();schedulePersist()}};
 document.getElementById('nailBrushSize').oninput=e=>brushSize=+e.target.value;
 document.getElementById('nailClearDrawing').onclick=()=>{ctx.clearRect(0,0,canvas.width,canvas.height);saveDrawing();schedulePersist()};
