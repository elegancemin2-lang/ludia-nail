...
window.LudiaSalonCloud=(()=>{
  ...
  async function signIn(email,password){if(!client)return fail('Supabase 설정이 필요해요');emitStatus({loading:true,error:null});const {error}=await client.auth.signInWithPassword({email:email.trim(),password});if(error){emitStatus({loading:false,connected:false,error:'login_failed'});throw error}}
  ...
  async function saveAppointment(payload){
    ...
    if(phone){const {data:found,error}=await client.from('ludia_customers').select('id').eq('salon_id',salonId).eq('phone_normalized',phone).limit(1);if(error)throw error;customerId=found?.[0]?.id||null}else{const {data:found,error}=await client.from('ludia_customers').select('id').eq('salon_id',salonId).eq('name',payload.customer).limit(2);if(error)throw error;if(found?.length===1)customerId=found[0].id}
  ...
  function bindUi(){
    ...
    $('#cloudLoginBtn')?.addEventListener('click',async()=>{const email=$('#cloudEmail')?.value||'',password=$('#cloudPassword')?.value||'';if(!email||!password)return callbacks.toast?.('이메일과 비밀번호를 입력해 주세요');try{await signIn(email,password);if($('#cloudPassword'))$('#cloudPassword').value=''}catch(error){console.warn(error)}});
