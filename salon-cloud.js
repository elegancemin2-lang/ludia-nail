...
window.LudiaSalonCloud=(()=>{
  ...
  const membershipText=row=>{
    if(!row)return'없음';
    if(row.kind==='amount')return `${row.name_snapshot} ${Math.max(0,row.remaining_amount||0).toLocaleString('ko-KR')}원`;
    return `${row.name_snapshot} ${Math.max(0,row.remaining_count||0)}회`;
  };
  function emitStatus(patch={}){
    Object.assign(state,patch);
    callbacks.onStatus?.({...state});
    renderAuthSheet();
  }
  function fail(message,error){
    console.error('[LUDIA Salon Cloud]',error||message);
    emitStatus({loading:false,error:message,realtime:'offline'});
  }
  async function init(nextCallbacks={}){
    callbacks=nextCallbacks;
    bindUi();
    emitStatus({loading:true,error:null});
    let cfg;
    try{
      const r=await fetch('/api/salon-config',{cache:'no-store'});
      cfg=await r.json();
    }catch(error){return fail('클라우드 설정을 확인할 수 없어요',error)}
    state.configured=Boolean(cfg?.configured);
    if(!state.configured)return emitStatus({loading:false,connected:false,error:null,realtime:'offline'});
    ...
    client.auth.onAuthStateChange((event,session)=>{
      if(event==='SIGNED_OUT'){resetCloudData();emitStatus({connected:false,loading:false,email:'',salonName:'',realtime:'offline',error:null});callbacks.onSignedOut?.();return}
      if(session?.user)setTimeout(()=>activateSession(session),0);
    });
    const {data:{session},error}=await client.auth.getSession();
    if(error)return fail('로그인 세션을 확인하지 못했어요',error);
    if(session?.user)await activateSession(session);
    else emitStatus({loading:false,connected:false,error:null,realtime:'offline'});
  }
  async function activateSession(session){
    user=session.user;state.email=user.email||'';emitStatus({loading:true,connected:false,error:null});
    await loadData();
  }
  async function loadData(){
    ...
    try{
      ...
      member=mine?.[0]||null;
      if(!member){
        salonId=null;resetCloudData(false);emitStatus({loading:false,connected:false,salonName:'',realtime:'offline',error:'salon_required'});return;
      }
      ...
      const staffMap=new Map(staffRecords.map(x=>[x.user_id,x.display_name]));
      const membershipMap=new Map();
      (membershipQ.data||[]).forEach(x=>{if(!membershipMap.has(x.customer_id))membershipMap.set(x.customer_id,x)});
      const customersRaw=customerQ.data||[];
      const customerMap=new Map(customersRaw.map(x=>[x.id,x]));
      const todayKey=kstDateKey(new Date());
      const keyDay=key=>Math.round((Date.parse(key+'T00:00:00Z')-Date.parse(todayKey+'T00:00:00Z'))/86400000);
      const customers=customersRaw.map((c,i)=>({
        cloudId:c.id,name:c.name,phone:c.phone||'연락처 없음',visit:c.visit_count||0,last:shortDate(c.last_visit_at),
        tags:Array.isArray(c.tags)?c.tags:[],membership:membershipText(membershipMap.get(c.id)),note:c.memo||'',img:`assets/nail_${i%5+1}.jpg`,
        preferences:c.preferences||{}
      }));
      const appointments=(appointmentQ.data||[]).map(a=>{
        const customer=customerMap.get(a.customer_id),start=new Date(a.starts_at),end=new Date(a.ends_at);
        return{
          id:a.id,cloudId:a.id,dayOffset:keyDay(kstDateKey(start)),time:kstTime(start),customer:a.customer_name_snapshot||customer?.name||'고객',
          service:a.service_name_snapshot||'시술',staff:staffMap.get(a.staff_user_id)||'미지정',staffUserId:a.staff_user_id,serviceId:a.service_id,
          duration:Math.max(5,Math.round((end-start)/60000)),amount:a.price||0,status:uiStatus(a.status),dbStatus:a.status,source:a.source||'manual',
          note:a.memo||'',membership:membershipText(membershipMap.get(a.customer_id)),last:shortDate(customer?.last_visit_at)
        }
      });
      const payload={appointments,customers,staffNames:staffRecords.map(x=>x.display_name),staffRecords:[...staffRecords],services:[...serviceRecords],salonName:salonQ.data?.name||'',email:user.email||''};
      lastPayload=payload;callbacks.applyData?.(payload);
      emitStatus({loading:false,connected:true,salonName:payload.salonName,email:user.email||'',error:null});
      subscribeRealtime();
  ...
  function queueReload(){
    clearTimeout(reloadTimer);reloadTimer=setTimeout(()=>loadData(),220);
  }
  function resetCloudData(clearIdentity=true){
    salonId=null;member=null;staffRecords=[];serviceRecords=[];lastPayload=null;
    if(channel&&client)client.removeChannel(channel).catch(()=>{});channel=null;
    if(clearIdentity)user=null;
  }
  async function signIn(email,password){
    if(!client)return fail('Supabase 설정이 필요해요');
    emitStatus({loading:true,error:null});
    const {error}=await client.auth.signInWithPassword({email:email.trim(),password});
    if(error){emitStatus({loading:false,connected:false,error:'login_failed'});throw error}
  }
  ...
  async function saveAppointment(payload){
    ...
    const service=serviceRecords.find(x=>x.name===payload.service)||null;
    const [hour,minute]=payload.time.split(':').map(Number),d=payload.date;
    ...
    if(error)throw error;await loadData();return true;
  }
  ...
  function renderAuthSheet(){
    const sheet=$('#cloudAuthSheet');if(!sheet)return;
    const signed=$('#cloudSignedIn'),login=$('#cloudLoginPane'),setup=$('#cloudSetupPane');
    if(signed)signed.classList.toggle('hidden',!state.connected);
    if(login)login.classList.toggle('hidden',state.connected||Boolean(user));
    if(setup)setup.classList.toggle('hidden',!user||state.connected);
    if($('#cloudAuthEmail'))$('#cloudAuthEmail').textContent=state.email||'';
    if($('#cloudSalonLabel'))$('#cloudSalonLabel').textContent=state.salonName||'샵 미연결';
    if($('#cloudRealtimeLabel'))$('#cloudRealtimeLabel').textContent=state.realtime==='live'?'실시간 동기화 중':state.realtime==='connecting'?'실시간 연결 중':'동기화 대기';
    const err=$('#cloudAuthError');if(err){err.textContent=state.error==='login_failed'?'이메일 또는 비밀번호를 확인해 주세요':state.error==='salon_required'?'로그인은 완료됐어요. 사용할 샵을 만들어 주세요.':state.error&&state.error!=='salon_required'?state.error:'';err.classList.toggle('hidden',!err.textContent)}
  }
  function bindUi(){
    $('#cloudAccountBtn')?.addEventListener('click',openAuthSheet);
    document.querySelectorAll('[data-close-cloud]').forEach(x=>x.addEventListener('click',closeAuthSheet));
    $('#cloudLoginBtn')?.addEventListener('click',async()=>{
      const email=$('#cloudEmail')?.value||'',password=$('#cloudPassword')?.value||'';if(!email||!password)return callbacks.toast?.('이메일과 비밀번호를 입력해 주세요');
      try{await signIn(email,password);if($('#cloudPassword'))$('#cloudPassword').value=''}catch(error){console.warn(error)}
    });
    $('#cloudLogoutBtn')?.addEventListener('click',()=>signOut());
    $('#cloudRefreshBtn')?.addEventListener('click',()=>refresh());
    $('#cloudCreateSalonBtn')?.addEventListener('click',async()=>{
      const name=$('#cloudSalonName')?.value.trim(),displayName=$('#cloudOwnerName')?.value.trim(),phone=$('#cloudSalonPhone')?.value.trim();
      if(!name||!displayName)return callbacks.toast?.('샵 이름과 내 표시 이름을 입력해 주세요');
      try{emitStatus({loading:true,error:null});await createSalon({name,displayName,phone});callbacks.toast?.('샵을 만들고 실시간 동기화를 시작했어요')}catch(error){fail('샵 생성에 실패했어요',error)}
    });
  }
  ...
  return{init,refresh,signIn,signOut,saveAppointment,updateAppointmentStatus,openAuthSheet,getState:()=>({...state}),getLastPayload:()=>lastPayload,isConnected:()=>state.connected};
})();
