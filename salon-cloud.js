...
window.LudiaSalonCloud=(()=>{
  ...
  function resetCloudData(clearIdentity=true){sessionGeneration++;salonId=null;state.salonId=null;state.connected=false;member=null;staffRecords=[];serviceRecords=[];lastPayload=null;if(channel&&client)client.removeChannel(channel).catch(()=>{});channel=null;if(clearIdentity)user=null}
  ...
  async function refresh(){if(!client||!user)throw new Error('authentication required');await loadData()}
  ...
  function bindUi(){
    ...
    $('#cloudLogoutBtn')?.addEventListener('click',()=>signOut());$('#cloudRefreshBtn')?.addEventListener('click',()=>refresh().catch(()=>callbacks.toast?.('새로고침에 실패했어요')));
  ...
  async function saveArtDesign({name,photoFile,price,listPrice,salePrice=0,monthKey='',time,tags=[],category='',materials=[],difficulty='보통',tech='',description='',status='draft',designState=null}){
