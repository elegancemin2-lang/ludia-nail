...
function cloudContext(){
 let failing=false,applyConnected=false,changed=0;
 ...
 const query=table=>new Proxy({}, {get(_,k){if(k==='then')return(resolve)=>resolve(failing?{error:new Error('offline')}:{data:rows[table]||[],error:null});return()=>query(table)}});
 const channel={on(){return this},subscribe(){return this}};
 const client={from:query,auth:{onAuthStateChange(){},async getSession(){return{data:{session:{user:{id:'qa-user',email:'qa@example.invalid'}}}}}},channel:()=>channel,removeChannel:async()=>{}};
 const c=vm.createContext({console:{error(){},warn(){}},setTimeout,clearTimeout,window:{supabase:{createClient:()=>client}},document:{querySelector:()=>null,querySelectorAll:()=>[]},fetch:async()=>({json:async()=>({configured:true,url:'https://example.invalid',anonKey:'test'})})});
 vm.runInContext(fs.readFileSync(path.join(root,'salon-cloud.js'),'utf8'),c);
 const cloud=c.window.LudiaSalonCloud;
 return{cloud,fail(){failing=true},async init(){await cloud.init({onSessionChanging(){changed++},applyData(){applyConnected=cloud.isConnected()}})},check(){return{applyConnected,changed}}};
}
