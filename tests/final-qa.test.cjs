...
function cloudContext(){
 let failing=false,applyConnected=false,changed=0;const ranges=[];
 ...
 return{cloud,ranges,fail(){failing=true},async init(){await cloud.init({getBookingDate:()=>new Date(2026,11,31,12),onSessionChanging(){changed++},applyData(){applyConnected=cloud.isConnected()}})},check(){return{applyConnected,changed}}};
}
